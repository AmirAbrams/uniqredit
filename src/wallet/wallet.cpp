...
static bool GetBoundAddress(CWallet* wallet, uint160 const& hash, CNetAddr& address) {
    ...
    std::set< std::pair<CNetAddr, uint64_t> > const& address_binds = wallet->get_address_binds();
    for (std::set< std::pair<CNetAddr, uint64_t> >::const_iterator checking = address_binds.begin();
        address_binds.end() != checking; checking++ ) {
        if (hash == Hash160(CreateAddressIdentification(checking->first, checking->second))) {
    ...
    if ("request-delegate" == name ) {
       ...
       CCoinControl coinControl;
        ...
        if (wallet->ReadRetrieveStringFromHashMap(sender_funded_tx, relayed_delegate_tx_id, true)) {
            uint256 relayed_delegate_hash =uint256S(relayed_delegate_tx_id);
...
CTransaction CreateDelegateBind( CNetAddr const& tor_address_parsed, boost::uint64_t const& nonce, uint64_t const& transferred, boost::uint64_t const& expiry, CUniqreditAddress const& recover_address_parsed) {
...
CTransaction CreateSenderBind(CNetAddr const& local_tor_address_parsed, boost::uint64_t const& received_delegate_nonce, uint64_t const& amount, uint64_t const& delegate_fee, boost::uint64_t const& expiry, CUniqreditAddress const& recover_address_parsed) {
...
void  InitializeDelegateBind(std::vector<unsigned char> const& delegate_key, uint64_t const& delegate_address_bind_nonce,
    ...
    CTransaction const rawTx = CreateDelegateBind(local, delegate_address_bind_nonce, nAmount, chainActive.Height() + escrow_expiry, recovery_address);
    uint160 delegate_id_hash;
    ...
    pwalletMain->store_hash_delegate(delegate_id_hash, delegate_key);
