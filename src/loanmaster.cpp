                ...
                for ( int i = 0; i < ctotal; i++ ){
                                        ...
                                        int64_t limit=0;
                                        {
                                                ...
                                                if (sqlite3_step(statement) == SQLITE_ROW){
                                                        ...
                                                        char* updatequery = sqlite3_mprintf("update RATINGS set TRUST = %lld, TXOUTCOUNT =%lld, TOTALOUT= %lld where ADDRESS = '%q'", trust, creditrating, isr$
                                                        ...
                                                        char * insertquery = sqlite3_mprintf("insert into RATINGS (ADDRESS, TRUST, CREDITRATING, REGISTERED, BAN, LIMIT) values ('%q',%lld,%lld,%lld,%lld)", a$
...
void addnewloanrequest(string address, CAmount amount, double premium, int expiry, int period, string message, string tx){
        ...
        char * insertquery = sqlite3_mprintf("insert into LOANREQUESTS (CHAINID, AMOUNT, PREMIUM, EXPIRY, PERIOD, TX) values ('%q', %lld, %f, %d, %d,'%q','%q')", address.c_str(), amount, premium, expiry, pe$
        rc = sqlite3_exec(rawdb, insertquery, callback, 0, &zErrMsg);
...
void addnewloan(string address, string reqaddress, string reqtx, CAmount amount, string requestid, string message){
        ...
        char * insertquery = sqlite3_mprintf("insert into LOANS (CHAINID, REQCHAINID , REQTX, AMOUNT, REQID, MSG) values ('%q', '%q', '%q', %d, '%q','%q')", address.c_str(), reqaddress.c_str(), reqtx.c_str($
        rc = sqlite3_exec(rawdb, insertquery, callback, 0, &zErrMsg);
